	...
	function mfn_builder_show() {
		...
		$mfn_std_section = array(
			...
			array(
				'id' 		=> 'column_margin',
				'type' 		=> 'select',
				'title' 	=> __('Column Margin', 'mfn-opts'),
			...
			array(
				...
				'desc'		=> __('Use this option to create One Page sites.<br /><br />For example: Your Custom ID is <strong>offer</strong> and you want to open this section, please use link: <strong>your-url/#offer</strong>', 'mfn-opts'),
				'class' 	=> 'small-text',
			),
			...
			'map' => array(
				...
				'fields'	=> array(
					...
					array(
						'id' 		=> 'latlng',
						'type' 		=> 'text',
						'title' 	=> __('Additional Markers | Lat,Lng', 'mfn-opts'),
						'desc' 		=> __('Separate Lat,Lang with <b>coma</b> [ , ]<br />Separate multiple Markers with <b>semicolon</b> [ ; ]<br />Example: <b>-33.88,151.21;-33.89,151.22</b>', 'mfn-opts'),
					),
				...
				if( key_exists($type, $_POST['mfn-items']) ){	
					foreach( (array) $_POST['mfn-items'][$type] as $attr_k => $attr ){
						...
						if( $attr_k == 'tabs'){
							...
							if( $seo_val && $seo_val != 1 ) $seo_content .= stripslashes( $seo_val ) ."\n\n";
								...
								foreach( $section['items'] as $item ){
									...
									if( function_exists( 'mfn_print_'. $item['type'] ) ){
										...
										if( isset( $item['fields']['classes'] ) ){
											$class .= ' '. $item['fields']['classes'];	// custom classes
										}
