...
if ( function_exists( 'add_theme_support' ) ) {
	...
	add_image_size( 'portfolio', 		640,  500,  true  ); 	// portfolio
...
function mfn_layout_ID(){
	...
	if( mfn_ID() && is_single() && get_post_type() == 'post' ){
		...
		// Theme Options | Layout select for Single Posts
...
function mfn_breadcrumbs(){
	...
	// Single -----------------------------------------
		...
		if( get_post_type() != 'post' ){
			...
			// Parents ------------
			if( $slug['slug'] == mfn_opts_get( 'portfolio-slug', 'portfolio-item' ) && $portfolio_page_id = mfn_wpml_ID( mfn_opts_get('portfolio-page') ) ){
...
// 		),
		...
		array(
			...
			'source'   				=> THEME_DIR .'/plugins/envato-wordpress-toolkit.zip',
			'required' 				=> false,
		),
...
// 		),
	...
	$config = array(
		...
		'message' 		=> '<div class="updated settings-error"><h3>&nbsp;&nbsp;If you are not sure about server\'s settings and limits, please activate <u>necessary plugins ONLY</u></h3></div>',	// Message to output right before the plugins table
		'strings'      	=> array(
