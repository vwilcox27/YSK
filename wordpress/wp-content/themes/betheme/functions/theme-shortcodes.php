		...
		$output = '<div class="zoom_box">';
			if( $link ) $output .= '<a href="'. $link .'" '. $target .'>';
				...
				$output .= '<div class="photo">';
					$output .= '<img class="scale-with-grid" src="'. $image .'" alt="" />';
				$output .= '</div>';
						...
						$output .= '<div class="item_wrapper">';
							...
							if( get_post_format() == 'quote'){
								...
								$output .= '<div class="image_frame scale-with-grid">';
									$output .= '<div class="image_wrapper">';
										$output .= '<a href="'. get_permalink() .'">';
											$output .= get_the_post_thumbnail( get_the_ID(), $thumb_size, array('class'=>'scale-with-grid' ) );
										$output .= '</a>';
									$output .= '</div>';
								$output .= '</div>';	
							}
		...
		$output = '<div class="pricing-box '. $classes .'">';
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				...
				if( $link ){
					...
					// button icon
					$icon = $icon ? '<span class="button_icon"><i class="'. $icon .'"></i></span>' : false;

					$output .= '<div class="plan-footer">';
						$output .= '<a href="'. $link .'" class="button  button_left button_theme button_js" '. $target .'>'. $icon .'<span class="button_label">'. $link_title .'</span></a>';
					$output .= '</div>';
				}
				...
				$output .= '<div class="desc_wrapper">';
					if( $number ){
						$output .= '<div class="number-wrapper">';
							...
							if( $label ) $output .= '<span class="label">'. $label .'</span>';
						$output .= '</div>';
					}
	...
	function sc_image( $attr, $content = null )
	{
		...
		if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
			...
			if( $link || $link_image ) {
				...
				$output .= '<div class="image_frame scale-with-grid'. $class_div .'" '. $margin .'>';
					$output .= '<div class="image_wrapper">';
				...
				$output .= '<div class="image_frame no_link scale-with-grid'. $class_div .'" '. $margin .'>';
					$output .= '<div class="image_wrapper">';
						$output .= '<img class="scale-with-grid" src="'. $src .'" alt="'. $alt .'" '.$width.' '.$height.' />';
					$output .= '</div>';
					if( $caption ) $output .= '<p class="wp-caption-text">'. $caption .'</p>';
	...
	function sc_quick_fact( $attr, $content = null )
	{
		...
		$output = '<div class="quick_fact '. $animate_math .'">';
			if( $animate ) $output .= '<div class="animate" data-anim-type="'. $animate .'">';
				...
				if( $number ) $output .= '<div class="number" data-to="'. $number .'">'. $number .'</div>';
				if( $title ) $output .= '<h3 class="title">'. $title .'</h3>';
				$output .= '<hr class="hr_narrow" />';
				$output .= '<div class="desc">'. do_shortcode($content) .'</div>';
