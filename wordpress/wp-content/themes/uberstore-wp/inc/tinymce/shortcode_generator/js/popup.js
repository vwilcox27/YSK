			...
			switch(name) {
				...
				case 'thb_button':
					var size = $('input[name='+name+'-size]:checked').val() || 'small',
							...
							blank = $('input[id='+name+'-blank]:checked').length;
					
					if (blank) { var target = "_blank"; } else { var target = "_self"; }
					if (rounded) { var round = 'true'; } else { var round = 'false';}
					
					content = '[thb_button link="'+link+'" color="'+color+'" icon="'+icon+'" size="'+size+'" animation="'+animation+'" target="'+target+'"]'+title+'[/thb_button]';
