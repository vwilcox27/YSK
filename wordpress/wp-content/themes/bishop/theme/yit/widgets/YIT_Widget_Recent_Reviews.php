...
class YIT_Widget_Recent_Reviews extends WP_Widget {
    ...
    function widget( $args, $instance ) {
        ...
        $id = ( isset( $instance['product'] ) && $instance['product'] != 'all' ) ? (int) $instance['product'] : 0;
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'product' ); ?>"><?php _e( 'Select Product', 'yit' ); ?></label>
            <select class="widefat" id="<?php echo $this->get_field_id( 'product' ) ?>" name="<?php echo $this->get_field_name( 'product' ) ?>">
                <option value="all"<?php selected( $instance['product'], 'all' ) ?>><?php _e( 'All products', 'yit' ); ?></option>
                <?php foreach ( $products as $product ): yit_debug( $product ); ?>
                    <option value="<?php echo $product->ID ?>"<?php selected( $instance['product'], $product->ID ) ?>><?php echo $product->post_title ?></option>
                <?php endforeach; ?>
            </select>
        </p>
