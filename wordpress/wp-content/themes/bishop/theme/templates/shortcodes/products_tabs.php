...
<div class="tabs-container products_tabs">
    <ul class="clearfix tabs">
        <?php for( $i = 1; isset( $atts['title_'.$i] ); $i++ ) :
            ...
            elseif ( $show == 'best_sellers' ) {
                $best_sellers          = 'yes';
                $atts['orderby_' . $i] = 'sales';
            }
            ...
            $sc[$yit_products_tabs_index] = '[products_slider title="' . $title . '" per_page="' . $per_page . '" product_type="' . $show . '" category="' . $category . '" orderby="' . $orderby . '" order="' . $order . '" layout="default" ]';
