	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				$plugin['source'] = urldecode(stripslashes($_GET[sanitize_key( 'plugin_source' )])); // Plugin source
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					/** Register the nag messages and prepare them to be processed */
                     $nt = ( isset( $this->strings['nag_type'] ) ) ? $this->strings['nag_type'] : '';
                     add_settings_error( 'tgmpa', 'tgmpa', $rendered, sanitize_html_class( strtolower( $nt ), 'updated' ) );
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				/** The plugin is from the WordPress repository */
                        ...
                        if(  isset( $plugin['repository'] ) )
                            $table_data[$i]['source'] = __( $plugin['repository'] , TGM_Plugin_Activation::$instance->domain );
                        else
