...
(function ($) {
    ...
    var page_list = {
        ...
        addPanelListener: function () {
            ...
            //form clear
            $('#layout-managment-liquid').on('click', '#panel-form-clear-submit', function (e) {
                e.preventDefault();
                var link = $(this),
                    r = confirm(  yit_layout_loc.confirm_reset );

                    if ( ! r ) {
                        return false;
                    }

                $('#panel-form').find('input[name="action"]').val('yit-layout-panel-clear');
                var p_id =  $('#panel-form').find('input[name="panel_id"]').val(),
                    p_type =  $('#panel-form').find('input[name="panel_type"]').val(),
                    p_model =  $('#panel-form').find('input[name="panel_model"]').val();
                $.ajax({
                    url     : ajaxurl,
                    data    : $('#panel-form').serialize(),
                    dataType: 'JSON',
                    ...
                    success : function (data) {
                        $('#message').html(data);
                        $('[data-id="'+ p_id +'"][data-type="'+ p_type +'"][data-model="'+ p_model +'"]').trigger('click');
                    },
                    error   : function (xhr, desc, e) {
                        console.log(xhr.responseText);
                    }
