...
function sf_check_location($opt, $post_id)
{
    ...
    if (is_home()) {
        ...
        $current_page_sf_menu = check_against_rule('wp_pages', 'home');
        if (!$show && $post_id) {
        ...
        if (!$show && is_front_page() /*&& isset($opt['page-front'])*/) {
            ...
            $current_page_sf_menu = check_against_rule('wp_pages', 'front');
        }
        ...
        if (is_front_page()) {
            ...
            if (is_category()) {
                ...
                $current_page_sf_menu = check_against_rule('cats', $catid);
                ...
                if (is_archive()) {
                    ...
                    $current_page_sf_menu = check_against_rule('wp_pages', 'archive');
                    ...
                    if (is_single()) {
                        ...
                        $current_page_sf_menu = check_against_rule('wp_pages', 'single');
                        if (!$show && $type != 'page' && $type != 'post') {
                            $show = isset($opt->location->cposts->$type);
                            $current_page_sf_menu = check_against_rule('cposts', $type);
                        }
                        ...
                        if (!$show) {
                            ...
                            foreach ($cats as $cat) {
                                ...
                                $current_page_sf_menu = check_against_rule('cats', $c_id);
                                unset($c_id);
                                unset($cat);
                            }
                        }
                        ...
                        if (is_404()) {
                            ...
                            $current_page_sf_menu = check_against_rule('wp_pages', 'forbidden');

                        } else {
                            if (is_search()) {
                                $show = isset($opt->location->wp_pages->search);
                                $current_page_sf_menu = check_against_rule('wp_pages', 'search');
...
function check_against_rule ($first_level, $sec_level) {
    $options = sf_get_options();
...
function sf_check_display_rule($opt, $isMobile, $post_id)
{
    ...
    $options = sf_get_options();
    $current_page_sf_menu = $options['sf_active_menu'];

    $show = sf_check_location($opt, $post_id, $options['sf_active_menu']);
