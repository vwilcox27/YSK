...
function sf_register_settings() {
    ...
    add_settings_field('sf_active_menu', "SUPERFLY MENUS", 'sf_active_menu_str', 'sf', 'sf_source', array('chapter' => 'Source'));
	...
	add_settings_field('sf_social_color', "Other Color", 'sf_social_color_str', 'sf', 'sf_appearance', array('subsection' => 'social'));
...
function sf_display_str() {
	...
	?>
	<h6>Control plugin displaying with next set of rules. Global setting for all Superfly menus.</h6>
	<p>
...
function sf_active_menu_str() {
	...
	echo "<h6>Click menu labels that you added to edit location rules. Or click cross icon to remove it.</h6>";
	if(!$menus){
    ...
    echo "<script>var sf_menus = {$options['sf_active_menu']}</script>";
